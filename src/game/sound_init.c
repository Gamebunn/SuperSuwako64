...
void set_background_music(u16 a, u16 seqArgs, s16 fadeTimer) {
    if (gResetTimer == 0 && seqArgs != sCurrentMusic) {
        ...
        if (!(gShouldNotPlayCastleMusic && seqArgs == SEQ_STREAMED_NEWCASTLE)) {
...
void play_shell_music(void) {
    play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, SEQ_STREAMED_NEWPOWERUP | SEQ_VARIATION), 0);
    sCurrentShellMusic = SEQUENCE_ARGS(4, SEQ_STREAMED_NEWPOWERUP | SEQ_VARIATION);
}
