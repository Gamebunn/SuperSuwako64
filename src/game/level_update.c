...
void init_mario_after_warp(void) {
    ...
    if (gCurrDemoInput == NULL) {
        ...
        if (gMarioState->flags & MARIO_METAL_CAP) {
            play_cap_music(SEQUENCE_ARGS(4, SEQ_STREAMED_NEWMETALCAP));
        }

        if (gMarioState->flags & (MARIO_VANISH_CAP | MARIO_WING_CAP)) {
            play_cap_music(SEQUENCE_ARGS(4, SEQ_STREAMED_NEWPOWERUP));
        }

#ifndef VERSION_JP
        if (gCurrLevelNum == LEVEL_BOB
            && get_current_background_music() != SEQUENCE_ARGS(4, SEQ_STREAMED_NEWSKY)
            && sTimerRunning != 0) {
            play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, SEQ_STREAMED_NEWSKY), 0);
        }
#endif
...
s16 music_changed_through_warp(s16 arg) {
    ...
    if (levelNum == LEVEL_BOB && levelNum == gCurrLevelNum && destArea == gCurrAreaIndex) {
        ...
        if (sp2C == SEQUENCE_ARGS(4, SEQ_STREAMED_NEWPOWERUP | SEQ_VARIATION)
            || sp2C == SEQUENCE_ARGS(4, SEQ_STREAMED_NEWPOWERUP)) {
            val4 = 0;
        }
    } else {
